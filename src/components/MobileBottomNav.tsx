...
export const MobileBottomNav: React.FC = () => {
  ...
  const badgeFor = (path: string) => path === '/chat' ? unreadChats : path === '/announcements' ? unreadNotifications : 0;
  ...
  return <>
    <nav className="md:hidden fixed bottom-3 left-3 right-3 z-40 rounded-2xl border border-slate-200/90 dark:border-slate-700/90 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl shadow-[0_12px_35px_rgba(15,23,42,.16)] px-2 py-2" aria-label={`${role} mobile navigation`}>
      <div className="flex items-center justify-between gap-1">
        {navigation.main.map(item => { const Icon = item.icon; const active = isActive(item.path); const badge = badgeFor(item.path); return <button key={item.name} onClick={() => item.onClick ? item.onClick() : navigate(item.path)} aria-label={item.name} className={`relative ${item.primary ? 'flex min-w-[76px] flex-col items-center justify-center gap-1 rounded-xl bg-emerald-600 px-2 py-2 text-white shadow-sm transition-all hover:bg-emerald-700 active:scale-95' : `flex min-w-[54px] flex-1 flex-col items-center justify-center gap-1 rounded-xl px-1 py-2 text-[10px] font-semibold transition-colors ${active ? 'bg-slate-100 text-slate-950 dark:bg-slate-800 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}`}><Icon className="h-5 w-5" /><span className="max-w-[72px] truncate">{item.name}</span>{badge > 0 && <span className="absolute right-1 top-0.5 grid min-h-4 min-w-4 place-items-center rounded-full bg-red-600 px-1 text-[9px] font-extrabold leading-none text-white ring-2 ring-white dark:ring-slate-900">{badge > 99 ? '99+' : badge}</span>}</button>; })}
      </div>
    </nav>
    ...
    {showMore && <div className="fixed inset-0 z-50 bg-slate-950/45 backdrop-blur-sm flex items-end" onClick={() => setShowMore(false)}>
      <div className="w-full rounded-t-[28px] border-t border-slate-200 bg-[#f7f7fb] p-5 shadow-2xl dark:border-slate-800 dark:bg-slate-900" onClick={event => event.stopPropagation()}>
        ...
        <div className="grid grid-cols-3 gap-2 py-2">{navigation.secondary.map(item => { const Icon = item.icon; const badge = badgeFor(item.path); return <button key={item.name} onClick={() => go(item.path)} className="relative flex flex-col items-center gap-2 rounded-2xl bg-white p-3 text-slate-700 transition-all dark:bg-slate-800 dark:text-slate-200">{badge > 0 && <span className="absolute right-2 top-2 grid min-h-4 min-w-4 place-items-center rounded-full bg-red-600 px-1 text-[9px] font-extrabold leading-none text-white">{badge > 99 ? '99+' : badge}</span>}<span className="rounded-xl bg-slate-100 p-2 dark:bg-slate-700"><Icon className="h-5 w-5" /></span><span className="text-center text-[11px] font-medium">{item.name}</span></button>; })}</div>
