...
import { streamHannaReply, isGeminiConfigured, generateSmartTitle, exportHannaArtifact, type HannaAttachment, type HannaArtifactFormat, type HannaMode } from '@/lib/hannaGemini';
...
import {
  Archive, ArrowUpRight, BookOpen, Check,   ChevronDown, ChevronLeft, ClipboardList, Copy, Download, ExternalLink, FileDown,
...
export default function HannaChatIntegrated() {
  ...
  useEffect(() => {
    if (autoResumeLatest && sessionsLoaded && !currentChatId && sessions.length > 0) {
  ...
  const exportLatestArtifact = async (format: HannaArtifactFormat) => {
    ...
    try {
      ...
      const result = await exportHannaArtifact({ title: sessionTitle, content: latestHannaMessage.content, format });
