...
import { streamHannaReply, isGeminiConfigured, generateSmartTitle, exportHannaArtifact, generateHannaImage, type HannaAttachment, type HannaArtifactFormat, type HannaMode } from '@/lib/hannaGemini';
  ...
  return <>
    ...
    <div className="flex h-[100dvh] min-h-0 overflow-hidden bg-[#f7f8fb] text-slate-900 dark:bg-[#080a0f] dark:text-slate-100">
      ...
      <main className="relative flex min-w-0 flex-1 flex-col overflow-hidden">
        ...
        <footer className="sticky bottom-0 z-20 shrink-0 border-t border-slate-200/80 bg-white/95 p-3 backdrop-blur-xl dark:border-white/10 dark:bg-[#080a0f]/95 sm:p-5"><div className="mx-auto max-w-3xl"><div className="relative mb-2 flex items-center justify-between text-[10px] text-slate-400"><span className="flex items-center gap-2 font-semibold text-emerald-600 dark:text-emerald-300"><span>Mode: {selectedMode.label}</span><select aria-label="Choose Hanna model" value={selectedModelId} onChange={event => setSelectedModelId(event.target.value)} className="max-w-[8.5rem] rounded-full border border-slate-200 bg-white px-2 py-1 text-[10px] font-bold text-slate-600 outline-none dark:border-white/10 dark:bg-white/5 dark:text-slate-300 sm:hidden">{SERVER_SUPPORTED_GEMINI_MODELS.map(model => <option key={model.id} value={model.id}>{model.label}</option>)}</select></span><button type="button" onClick={() => setShowModeMenu(value => !value)} className="inline-flex items-center gap-1 font-bold text-emerald-600 sm:hidden" aria-expanded={showModeMenu}>Change mode <ChevronDown className={`h-3 w-3 transition ${showModeMenu ? 'rotate-180' : ''}`} /></button>{showModeMenu && <div className="absolute bottom-7 right-0 z-30 w-64 rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl dark:border-white/10 dark:bg-[#11151d] sm:hidden">{HANNA_MODE_OPTIONS.map(option => <button type="button" key={option.value} onClick={() => { setHannaMode(option.value); setShowModeMenu(false); }} className={`flex w-full items-start gap-2 rounded-xl px-3 py-2 text-left ${option.value === hannaMode ? 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-300' : 'hover:bg-slate-100 dark:hover:bg-white/10'}`}><option.icon className="mt-0.5 h-4 w-4 shrink-0" /><span><span className="block text-xs font-bold">{option.label}</span><span className="mt-0.5 block text-[10px] text-slate-400">{option.description}</span></span></button>)}</div>}</div>{(attachments.length > 0 || uploading) && <div className="mb-2 flex max-w-full flex-wrap gap-2" aria-label="Selected attachments">{attachments.map(att => att.mimeType.startsWith('image/') ? <div key={att.url} className="relative h-16 w-16 overflow-hidden rounded-xl border border-slate-200 shadow-sm dark:border-white/10"><img src={att.url} alt="Selected image" className="h-full w-full object-cover" /><button type="button" onClick={() => setAttachments(prev => prev.filter(item => item.url !== att.url))} aria-label={`Remove ${att.name}`} className="absolute right-1 top-1 grid h-5 w-5 place-items-center rounded-full bg-black/70 text-white hover:bg-red-500"><X className="h-3 w-3" /></button></div> : <div key={att.url} className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-50 px-2.5 py-2 text-[10px] font-bold text-slate-600 dark:border-white/10 dark:bg-white/5 dark:text-slate-300"><Paperclip className="h-3.5 w-3.5 text-emerald-500" /> Attached file<button type="button" onClick={() => setAttachments(prev => prev.filter(item => item.url !== att.url))} aria-label={`Remove ${att.name}`}><X className="h-3 w-3 text-slate-400 hover:text-red-500" /></button></div>)}{uploading && <span className="inline-flex items-center gap-1 rounded-xl border border-slate-200 bg-slate-50 px-2.5 py-2 text-[10px] text-slate-400 dark:border-white/10 dark:bg-white/5"><span className="h-3 w-3 animate-pulse rounded-full bg-emerald-500" /> Preparing attachment</span>}</div>}<form onSubmit={event => { event.preventDefault(); void send(); }} className="relative flex items-end gap-2 rounded-[25px] border border-slate-200 bg-white p-2 shadow-[0_12px_40px_rgba(15,23,42,.08)] focus-within:border-emerald-400 focus-within:ring-4 focus-within:ring-emerald-500/10 dark:border-white/10 dark:bg-white/[.05]"><input ref={fileRef} type="file" multiple accept="image/*,video/*,audio/*,.pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.txt,.csv" className="hidden" onChange={handleFile} /><input ref={cameraRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={handleFile} /><div className="relative shrink-0"><button type="button" onClick={() => setShowPromptMenu(value => !value)} aria-expanded={showPromptMenu} aria-label="Add to Hanna chat" className="grid h-9 w-9 place-items-center rounded-full text-slate-400 hover:bg-slate-100 hover:text-emerald-600 dark:hover:bg-white/10"><Plus className={`h-5 w-5 transition-transform ${showPromptMenu ? 'rotate-45' : ''}`} /></button>{showPromptMenu && <div className="fixed inset-x-3 bottom-24 z-30 w-auto rounded-[28px] border border-slate-200 bg-white p-3 shadow-2xl dark:border-white/10 dark:bg-[#11151d] sm:absolute sm:inset-x-auto sm:bottom-12 sm:left-0 sm:w-72 sm:rounded-2xl sm:p-2"><div className="mb-2 flex items-center justify-between px-2 text-xs font-black"><span>Add to Hanna</span><button type="button" onClick={() => setShowPromptMenu(false)} className="rounded-full p-1 text-slate-400 hover:bg-slate-100 dark:hover:bg-white/10" aria-label="Close add menu"><X className="h-4 w-4" /></button></div><div className="grid grid-cols-3 gap-2 border-b border-slate-100 pb-3 dark:border-white/10"><button type="button" onClick={() => { setShowPromptMenu(false); cameraRef.current?.click(); }} className="flex flex-col items-center gap-1 rounded-2xl bg-slate-50 px-2 py-3 text-[10px] font-bold hover:bg-emerald-50 dark:bg-white/5 dark:hover:bg-emerald-500/10"><Camera className="h-5 w-5 text-emerald-500" />Camera</button><button type="button" onClick={() => { setShowPromptMenu(false); fileRef.current?.click(); }} className="flex flex-col items-center gap-1 rounded-2xl bg-slate-50 px-2 py-3 text-[10px] font-bold hover:bg-emerald-50 dark:bg-white/5 dark:hover:bg-emerald-500/10"><ImageIcon className="h-5 w-5 text-amber-500" />Photos</button><button type="button" onClick={() => { setShowPromptMenu(false); fileRef.current?.click(); }} className="flex flex-col items-center gap-1 rounded-2xl bg-slate-50 px-2 py-3 text-[10px] font-bold hover:bg-emerald-50 dark:bg-white/5 dark:hover:bg-emerald-500/10"><Paperclip className="h-5 w-5 text-blue-500" />Files</button></div><button type="button" onClick={() => { setHannaMode(researchEnabled ? 'studying' : 'web_search'); setShowPromptMenu(false); }} className="mt-2 flex w-full items-center justify-between rounded-2xl px-3 py-2.5 text-left text-xs font-bold hover:bg-slate-100 dark:hover:bg-white/10"><span className="flex items-center gap-2"><Globe2 className="h-4 w-4 text-emerald-500" />Web search</span><span className={`h-5 w-9 rounded-full p-0.5 ${researchEnabled ? 'bg-blue-500' : 'bg-slate-300 dark:bg-slate-700'}`}><span className={`block h-4 w-4 rounded-full bg-white shadow transition ${researchEnabled ? 'translate-x-4' : ''}`} /></span></button><button type="button" onClick={() => { void searchImages(); setShowPromptMenu(false); }} className="flex w-full items-center gap-2 rounded-2xl px-3 py-2.5 text-left text-xs font-bold hover:bg-slate-100 dark:hover:bg-white/10"><ImageIcon className="h-4 w-4 text-amber-500" />Find visual references</button><button type="button" onClick={() => { setShowModeMenu(true); setShowPromptMenu(false); }} className="flex w-full items-center gap-2 rounded-2xl px-3 py-2.5 text-left text-xs font-bold hover:bg-slate-100 dark:hover:bg-white/10"><Code2 className="h-4 w-4 text-violet-500" />Choose coding or study mode</button><button type="button" onClick={() => { setIsSourceDrawerOpen(true); setShowPromptMenu(false); }} className="flex w-full items-center gap-2 rounded-2xl px-3 py-2.5 text-left text-xs font-bold hover:bg-slate-100 dark:hover:bg-white/10"><Library className="h-4 w-4 text-blue-500" />Recent sources and source cards</button><div className="my-2 border-t border-slate-100 dark:border-white/10" />{HANNA_CREATION_ACTIONS.map(action => <button type="button" key={action.label} onClick={() => applyCreationPrompt(action.prompt, action.label === 'Create image' ? 'image' : action.label === 'Create slides' ? 'slides' : undefined)} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-xs font-bold hover:bg-slate-100 dark:hover:bg-white/10"><action.icon className="h-4 w-4 text-amber-500" />{action.label}</button>)}</div>}</div><textarea ref={composerRef} value={inputValue} onChange={event => setInputValue(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send(); } }} rows={1} placeholder="Assign a task or ask anything" className="max-h-32 min-h-9 flex-1 resize-none bg-transparent px-2 py-2 text-sm outline-none placeholder:text-slate-400" disabled={isGenerating} />{isGenerating ? <button type="button" onClick={stop} className="grid h-9 w-9 place-items-center rounded-full bg-slate-900 text-white dark:bg-white dark:text-slate-900" aria-label="Stop generating"><StopCircle className="h-4 w-4 text-emerald-400" /></button> : <button type="submit" disabled={!inputValue.trim() && !attachments.length || uploading} className="grid h-9 w-9 place-items-center rounded-full bg-emerald-600 text-white shadow-md shadow-emerald-600/20 transition hover:scale-105 disabled:opacity-40" aria-label="Send message"><Send className="h-4 w-4" /></button>}</form></div></footer>
      </main>
