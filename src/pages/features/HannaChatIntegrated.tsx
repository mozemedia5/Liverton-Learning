...
import { streamHannaReply, isGeminiConfigured, generateSmartTitle, exportHannaArtifact, type HannaAttachment, type HannaArtifactFormat } from '@/lib/hannaGemini';
...
export default function HannaChatIntegrated() {
  ...
  const exportLatestArtifact = async (format: HannaArtifactFormat) => {
    if (!latestHannaMessage?.content || artifactFormat) return;
    ...
    try {
      ...
      toast.success(`${result.format.toUpperCase()} downloaded.`);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Hanna could not create this document.');
