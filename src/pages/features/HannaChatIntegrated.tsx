...
export default function HannaChatIntegrated() {
  ...
  const createSession = async () => {
    ...
    const ref = await addDoc(collection(db, 'hanna_chats'), { userId: currentUser.uid, title: 'New conversation', createdAt: serverTimestamp(), updatedAt: serverTimestamp(), messageCount: 0 });
  ...
  const send = async (preset?: string) => {
    ...
    try {
      ...
      const title = existingSession?.title === 'New conversation' || !existingSession?.title
