...
function AttachmentPreview({ attachment, onPreview }: { attachment: HannaAttachment; onPreview: (attachment: HannaAttachment) => void }) {
  ...
  if (isImage) return <button type="button" onClick={() => onPreview(attachment)} className="group overflow-hidden rounded-xl border border-slate-200/70 bg-white text-left dark:border-white/10 dark:bg-[#111115]"><img src={attachment.url} alt={attachment.name} className="h-24 w-32 object-cover transition group-hover:scale-105" /><span className="block max-w-32 truncate px-1.5 py-1 text-[9px] font-bold text-slate-600 dark:text-slate-300">{attachment.name}</span></button>;
