...
const items = [
  { title: 'Modules', description: 'Learn, teach, and revisit structured learning paths.', path: '/student/courses', icon: BookOpen, tone: 'lavender' },
...
export default function MoreHub() {
  ...
  return <div className="liv-page">
    ...
    <div className="liv-grid-3">{items.map(({ title, description, path, icon: Icon, tone }) => <button key={title} className={`liv-card liv-more-card liv-accent-${tone === 'lavender' ? 'lavender' : tone === 'lime' ? 'teal' : tone === 'teal' ? 'teal' : tone === 'peach' || tone === 'orange' ? 'peach' : 'soft'}`} onClick={() => navigate(path)} style={{ textAlign:'left' }}><span className="liv-row-icon"><Icon size={18} /></span><h3 style={{ margin:'18px 0 7px', fontFamily:'Space Grotesk', fontSize:18 }}>{title}</h3><p style={{ margin:0, color:'var(--liv-muted)', fontSize:12, lineHeight:1.5 }}>{description}</p><span style={{ display:'flex', alignItems:'center', gap:5, marginTop:18, fontSize:11, fontWeight:700 }}>Open <ArrowUpRight size={14} /></span></button>)}</div>
  </div>;
}
