...
const items = [
  ...
  { title: 'Liv Fund', description: 'Discover and support community-led opportunities.', path: '/features/liv-fund', icon: HeartHandshake, tone: 'peach', underDevelopment: true },
  { title: 'Liv Mart', description: 'Find useful products, services, and creator offers.', path: '/features/liv-mart', icon: ShoppingBag, tone: 'orange', underDevelopment: true },
...
export default function MoreHub() {
  ...
  return <div className="liv-page">
    ...
    <div className="liv-grid-3">{items.map(({ title, description, path, educatorPath, icon: Icon, tone, underDevelopment }) => <button key={title} className={`liv-card liv-more-card liv-accent-${tone === 'lavender' ? 'lavender' : tone === 'lime' ? 'teal' : tone === 'teal' ? 'teal' : tone === 'peach' || tone === 'orange' ? 'peach' : 'soft'}`} onClick={() => navigate(userRole === 'teacher' && educatorPath ? educatorPath : path)} style={{ textAlign:'left', position:'relative' }}>{underDevelopment && <span style={{ position:'absolute', right:14, top:14, borderRadius:999, background:'#2563eb', color:'#fff', padding:'5px 8px', fontSize:9, fontWeight:800, letterSpacing:'.08em', textTransform:'uppercase' }}>Under Development</span>}<span className="liv-row-icon"><Icon size={18} /></span><h3 style={{ margin:'18px 0 7px', fontFamily:'Space Grotesk', fontSize:18 }}>{title}</h3><p style={{ margin:0, color:'var(--liv-muted)', fontSize:12, lineHeight:1.5 }}>{description}</p><span style={{ display:'flex', alignItems:'center', gap:5, marginTop:18, fontSize:11, fontWeight:700 }}>{underDevelopment ? 'Learn more' : 'Open'} <ArrowUpRight size={14} /></span></button>)}</div>
  </div>;
}
