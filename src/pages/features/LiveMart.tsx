...
import { getTeamProjectsForEconomicFeatures, getLivMartListings, createLivMartListing, createPendingLivMartOrder, createProjectVerification } from '@/services/livFundMartService';
...
export default function LiveMart() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [listings, teams] = await Promise.all([getLivMartListings(), getAllTeams()]);
  ...
  const submitVerification = async () => {
    ...
    setSaving(true);
    try {
      await createProjectVerification(selectedProject, currentUser.uid);
      toast.success('Project submitted for verification. It must be approved before LivMart listing.');
      await load();
    } catch (error) { toast.error(error instanceof Error ? error.message : 'Could not submit verification'); }
