...
export async function uploadToCloudinary(
  ...
  options: CloudinaryUploadOptions | ((percent: number) => void) = {}
): Promise<string> {
  const normalizedOptions: CloudinaryUploadOptions =
    typeof options === 'function' ? { onProgress: options } : options;
  const { onProgress, showErrorToast = true } = normalizedOptions;
  ...
  try {
    ...
    trackAssetInFirestore(data, file, type, normalizedOptions);
