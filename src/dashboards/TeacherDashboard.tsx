...
export default function TeacherDashboard() {
  ...
  return <div className="liv-page">
    ...
    <div className="liv-grid-4" style={{ marginTop:14 }}>
      {[['Published modules','12','+3 this month',BookOpen],['Active learners','248','+18% this week',Users],['Average rating','4.8 / 5','Across your library',Star],['Earnings','KSh 84,600','Ready to withdraw',WalletCards]].map(([label,value,note,Icon]) => <div className="liv-card liv-stat" key={String(label)}><div className="liv-stat-top"><span>{label}</span><span className="liv-stat-icon"><Icon size={16} /></span></div><strong className="liv-stat-value">{value}</strong><span className="liv-stat-note">{note}</span></div>)}
    </div>
